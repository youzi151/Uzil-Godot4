...
## 結果覆寫 函式
var _result_overwrite_fn : Callable
...
func _init () :
	...
	self._result_overwrite_fn = func (val) : return val
...
## 設置 結果覆寫 函式
func set_result_overwrite_fn (fn : Callable) :
	self._result_overwrite_fn = fn
...
func del_route (route, is_update = true) :
	...
	# 呼叫 自己 的 刷新事件
	var update_result = self._result_overwrite_fn.call(null)
...
func clear () :
	...
	for each in to_rm_datas :
		each.on_update.emit(self._result_overwrite_fn.call(null))
...
#	print("update : %s" % data.route)
	...
	# 每筆 子資料
	for sub_data in data.sub_datas :
		# 若不是 終點的資料 則 遞迴刷新 該子資料
		if sub_data != _until_data :
	...
	# 是否 總結值 有變更
	var is_sum_val_changed : bool = data.summary_val != new_sum_val
	...
	if not is_recursive :
		...
		for each in to_call_on_update :
			var update_result = self._result_overwrite_fn.call(each.summary_val)
			# 呼叫 刷新事件
	...
	if is_sum_val_changed :
		...
		# 若 已脫離 遞迴
			...
			# 直接 呼叫 刷新事件
			var update_result = self._result_overwrite_fn.call(data.summary_val)
			data.on_update.emit(update_result)
...
func _get_data (route_or_data, is_new_if_not_exist = true) :
	...
	# 若 路由 非字串 則 視為已處理 直接返回
	if typeof(route_or_data) != TYPE_STRING :
		return route_or_data
...
func _sum_val (data) :
	...
	var sum_val = self._summary_val_fn.call(data.val, sub_vals)
