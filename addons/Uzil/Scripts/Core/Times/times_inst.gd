...
## 設置 在背景計時
func set_timing_in_background (is_timing_in_background, _user = null, _priority = 0) :
	self._is_timing_in_background_vals.set_data(is_timing_in_background, _user, _priority)
...
func _pause (is_auto_pause) :
	...
	if is_auto_pause : 
		self._is_timing_vals.set_data(false, "_auto", 1)
	else : 
