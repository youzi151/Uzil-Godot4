...
func post_form (url: String, form_dict: Dictionary, opts := {}) :
	...
	return await self._do_request(func(http_node):
		# 等候器
...
func post_data (url: String, data: Dictionary, opts := {}) :
	...
	# 送出
	var post_result = await self._do_request(func(http_node):
...
## GET
func get_data (url: String, query_dict := {}, opts := {}) :
	# 表頭
	var headers := []
	# 超時
	var timeout := 10
	...
	for key in query_dict.keys():
		...
		var encoded_value = str(query_dict[key]).uri_encode()
		query_string_parts.push_back("%s=%s" % [encoded_key, encoded_value])
	
	var query_string : String = "&".join(query_string_parts)
	...
	# 送出
	var get_result = await self._do_request(func(http_node):
		...
		# 送出請求
		var error := request.request(full_url, headers, HTTPClient.METHOD_GET)
		# 若 沒有成功送出
		if error != OK :
			push_error("An error occurred in the HTTP request : %s" % [error])
			response.err = FAILED
...
func _do_request (req_fn: Callable, opts := {}) :
