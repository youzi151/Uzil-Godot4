...
func unprehold_preset (preset_path: String) :
	# 試取得 為 檔案路徑
	var file_path = self.get_file_path_if_is(preset_path, false)
...
func collect_preset_info (preset_path: String, path_to_info: Dictionary = {}, preset_path_to_file_path := {}) :
	...
	if has_requires : 
		...
		for path in preset_info["requires"] :
			var f_path = self.get_file_path_if_is(path, false)
...
## 取得 檔案路徑 若為檔案路徑
func get_file_path_if_is (full_path: String, return_original_if_is_not := true) :
