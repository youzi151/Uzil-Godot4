...
func test_simple () :
	...
	var Res_FILE_PATH = Res.PATH.replace("res://", "file://")
	
	res_mgr.is_debug = true
	res_mgr.built_in_loader.is_debug = true
	
	var is_success = await res_mgr.prehold_preset(Res_FILE_PATH.path_join("_test/test_res_preset_target.json"))
	G.print("prehold result : %s" % is_success)
	
	var res_target = await res_mgr.hold(Res_FILE_PATH.path_join("_test/res/res_target.txt"))
	G.print(res_target.res)
	
	var res_base = await res_mgr.hold(Res_FILE_PATH.path_join("_test/res/res_base.txt"))
	...
	res_mgr.unprehold_preset(Res_FILE_PATH.path_join("_test/test_res_preset_target.json"))
	...
	res_base = await res_mgr.hold(Res_FILE_PATH.path_join("_test/res/res_base.txt"))
